...
namespace Veracity.Services.Api
{
    ...
    public interface ICompaniesDirectory : IVeracityService
    {
        [Get]
        [IRoute("companies/{id}")]
        [ServiceDescription("Get the detailed company description")]
        ...
        [Get]
        [IRoute("companies/{id}/users")]
        [ServiceDescription("Get users affiliated with the company. Paged query: uses 0 based page index")]
        [AccessControllGate(AccessControllTypes.ServiceThenUser, RoleTypes.ReadDirectoryAccess)]
